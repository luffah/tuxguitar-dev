...
public class Caret {
	...
	private boolean isSelectionMarked;/* Check if user activate the selection */
	private boolean isSelectionMarkOpenned;/* Check if user is selecting */
	private boolean multiSelectionDirectionLeftToRight;
	private TGBeat multiSelectionBeatFirst;
	private TGBeat multiSelectionBeatLast;
	private TGMeasureImpl multiSelectionStartMeasure;
	private TGMeasureImpl multiSelectionEndMeasure;
	...
	public Caret(Tablature tablature) {
		...
		this.changes = false;
		this.isSelectionMarked = false;
		this.isSelectionMarkOpenned = false;
		this.multiSelectionDirectionLeftToRight = true;
		this.multiSelectionStartMeasure = null;
		this.multiSelectionEndMeasure = null;
		this.multiSelectionBeatFirst = null;
		this.multiSelectionBeatLast = null;
	}
	...
	public void moveTo(TGTrackImpl selectedTrack,
			TGMeasureImpl selectedMeasure, TGBeat selectedBeat, int string) {
		...
		if (!TuxGuitar.instance().getPlayer().isRunning()) {
			if (this.selectedMeasure != null
				...
				if ((layout.getStyle() & ViewLayout.DISPLAY_TABLATURE) != 0) {
					...
					int stringSpacing = this.tablature.getViewLayout()
							.getStringSpacing();
					...
					int y = this.selectedMeasure.getPosY()
							...
							+ ((this.string * stringSpacing) - stringSpacing)
							- 7;
				...
				if ((layout.getStyle() & ViewLayout.DISPLAY_SCORE) != 0) {
					int line = this.tablature.getViewLayout()
							.getScoreLineSpacing();
					int leftSpacing = beat.getMeasureImpl().getHeaderImpl()
							.getLeftSpacing(layout);
	...
	public void moveLeft() {
		if (getSelectedBeat() != null) {
			TGMeasureImpl measure = getMeasure();
			TGVoice voice = getSongManager().getMeasureManager()
					.getPreviousVoice(measure.getBeats(), getSelectedBeat(),
							getVoice());
			TGBeat beat = (voice != null ? voice.getBeat() : null);
			if (beat == null) {
	...
	private void updateDuration() {
		if (this.selectedBeat != null
			...
			this.selectedBeat.getVoice(getVoice()).getDuration()
					.copy(this.selectedDuration);
		}
	}
	...
	private void checkTransport() {
		TuxGuitar.instance().getTransport()
				.gotoMeasure(getMeasure().getHeader());
	}
	...
	// Selection is a set of TGBeats marked and accessible by the user
	// Selection methods :
	// openSelectionMark() means to create selection from the current position
	// isSelectionOpenned() means the selection is created but not terminated
	// closeSelectionMark() means to mark the end of the selection
	// isSelectionClosed() means the selection is created and that the beat can
	// be used properly
	// getSelectionBeats() provide the array of selected beats
	// paintSelection(ViewLayout layout, TGPainter painter) draws the square representing the selection
	// [paintSelection called by paintCaret but can be called on higher level]
	/**
	 * @author : luffah date : 01/01/2016
	 **/
	public boolean isSelectionOpenned() {
		return this.isSelectionMarked && this.isSelectionMarkOpenned;
	}
	...
	private void addToSelectionMark() {
		if (this.multiSelectionBeatFirst.getStart() == this.multiSelectionBeatLast
				.getStart()) {
			if (this.multiSelectionBeatFirst.getStart() < this.selectedBeat
					.getStart()) {
				this.multiSelectionDirectionLeftToRight = true;
				this.multiSelectionBeatLast = this.getSelectedBeat();
				this.multiSelectionEndMeasure = this.selectedMeasure;
			} else if (this.multiSelectionBeatFirst.getStart() > this.selectedBeat
					.getStart()) {
				this.multiSelectionDirectionLeftToRight = false;
				this.multiSelectionBeatFirst = this.getSelectedBeat();
				this.multiSelectionStartMeasure = this.selectedMeasure;
			}
		} else if (this.multiSelectionDirectionLeftToRight) {
			if (this.multiSelectionBeatFirst.getStart() <= this.selectedBeat
					.getStart()) {
				this.multiSelectionBeatLast = this.selectedBeat;
				this.multiSelectionEndMeasure = this.selectedMeasure;
			}
		} else {
			if (this.multiSelectionBeatLast.getStart() >= this.selectedBeat
					.getStart()) {
				this.multiSelectionBeatFirst = this.selectedBeat;
				this.multiSelectionStartMeasure = this.selectedMeasure;
			}
		}
	}

	public void openSelectionMark() {
		this.isSelectionMarked = true;
		this.isSelectionMarkOpenned = true;
		this.multiSelectionBeatFirst = this.selectedBeat;
		this.multiSelectionStartMeasure = this.selectedMeasure;
		this.multiSelectionBeatLast = this.selectedBeat;
		this.multiSelectionEndMeasure = this.selectedMeasure;

		this.addToSelectionMark();
	}

	public void closeSelectionMark() {
		this.isSelectionMarkOpenned = false;
	}

	public void unsetSelectionMark() {
		this.isSelectionMarked = false;
	}

	public ArrayList getSelectionBeats() {
		ArrayList beats = new ArrayList();
		if ((this.multiSelectionBeatFirst != null)
				&& (this.multiSelectionBeatLast != null)) {
			TGBeat btmp = this.multiSelectionBeatFirst;
			TGBeat btmp_end = this.multiSelectionBeatLast;
			TGMeasure measure = this.multiSelectionStartMeasure;

			if (btmp.getStart() == this.multiSelectionBeatLast.getStart()) {
				beats.add(btmp);
			} else {
				while ((btmp != null)
						&& (btmp.getStart() < this.multiSelectionBeatLast
								.getStart())) {
					beats.add(btmp);
					if (measure != null) {
						btmp = getSongManager().getMeasureManager()
								.getNextBeat(measure.getBeats(), btmp);
						if (btmp != null) {
						} else {
							measure = getSongManager().getTrackManager()
									.getNextMeasure(measure);
							if (measure != null) {
								btmp = getSongManager().getMeasureManager()
				...
				beats.add(this.multiSelectionBeatLast);
			}
		}
		return beats;
	}

	private void paintSelection(ViewLayout layout, TGPainter painter) {
		if (!TuxGuitar.instance().getPlayer().isRunning()) {
			if (this.selectedMeasure != null
					&& this.selectedBeat instanceof TGBeatImpl) {
				TGBeatImpl beat = (TGBeatImpl) this.selectedBeat;
				...
				int width = 14;
				int height = 14;
				if (this.isSelectionMarked) {// draw rectangle indicating
					// selected beats
					int leftSpacing = beat.getMeasureImpl().getHeaderImpl()
							.getLeftSpacing(layout);
					...
					int ya;
					int yb;
					int linewidth_sel = width / 5;

					painter.initPath();
					painter.setForeground(TuxGuitar.instance().getDisplay()
							.getSystemColor(SWT.COLOR_LIST_FOREGROUND));

					if (this.isSelectionMarkOpenned) {
						this.addToSelectionMark();
						painter.setBackground(TuxGuitar.instance().getDisplay()
					...
					TGBeatImpl b1 = (TGBeatImpl) this.multiSelectionBeatFirst;
					TGBeatImpl b2 = (TGBeatImpl) this.multiSelectionBeatLast;
					int posx1 = this.multiSelectionStartMeasure.getPosX()
							+ b1.getPosX() + b1.getSpacing() + leftSpacing;
					int posx2 = this.multiSelectionEndMeasure.getPosX()
							+ b2.getPosX() + b2.getSpacing() + leftSpacing;
					int minposy1 = this.multiSelectionStartMeasure.getPosY()
							+ this.multiSelectionStartMeasure.getTs()
									.getPosition(TGTrackSpacing.POSITION_TOP);
					int maxposy1 = this.multiSelectionStartMeasure.getPosY()
							+ this.multiSelectionStartMeasure
									.getTs()
									.getPosition(TGTrackSpacing.POSITION_BOTTOM);
					if (this.multiSelectionStartMeasure.getPosY() != this.multiSelectionEndMeasure
							.getPosY()) {
						int minposy2 = this.multiSelectionEndMeasure.getPosY()
								+ this.multiSelectionEndMeasure.getTs()
										.getPosition(
												TGTrackSpacing.POSITION_TOP);
						int maxposy2 = this.multiSelectionEndMeasure.getPosY()
								+ this.multiSelectionEndMeasure.getTs()
										.getPosition(
												TGTrackSpacing.POSITION_BOTTOM);
						int rightSpacingPosx = layout.getWidth()
								- beat.getMeasureImpl().getHeaderImpl()
										.getRightSpacing(layout);
						// add rectangle for beginning of selection
						xa = posx1 - (width / 2);
						polygons.add(new int[] { xa, maxposy1, xa, minposy1,
								rightSpacingPosx, minposy1, rightSpacingPosx,
								maxposy1 });
						// add rectangle for end of selection
						xb = posx2 + width;
						polygons.add(new int[] { leftSpacing, maxposy2,
								leftSpacing, minposy2, xb, minposy2, xb,
								maxposy2 });
						// add rectangle for middle of selection
						if (maxposy1 < (minposy2 - 2 * height))
							polygons.add(new int[] { leftSpacing,
									maxposy1 + height, leftSpacing,
									minposy2 - height, rightSpacingPosx,
									minposy2 - height, rightSpacingPosx,
									maxposy1 + height });
						...
						xb = posx2 + width;
						polygons.add(new int[] { xa, maxposy1, xa, minposy1,
								xb, minposy1, xb, maxposy1 });
					}
					Iterator it = polygons.iterator();
					int[] polygon;
					while (it.hasNext()) {
						polygon = (int[]) it.next();
						painter.setLineWidth(linewidth_sel);
						painter.setAlpha(75);
						painter.drawPolygon(polygon);

						// draw background of the selection
						polygon[0] = polygon[0] + linewidth_sel;
						polygon[4] = polygon[4] - (linewidth_sel);
						polygon[3] = polygon[3] + linewidth_sel;
						polygon[1] = polygon[1] - (linewidth_sel);
								...
								" '"
										+ TuxGuitar
												...
												.getKeyBindingForAction(
														"action.caret.select")
										+ "' again to accept selection ",
								leftSpacing,
								this.selectedMeasure.getTs().getPosition(
										TGTrackSpacing.POSITION_TOP), false);
					} else {
						...
						painter.drawString(
								" '"
										+ TuxGuitar
												...
												.getKeyBindingForAction(
														"action.caret.select")
										+ "' again to disable selection / '"
										+ TuxGuitar
												.instance()
												.getkeyBindingManager()
												.getKeyBindingForAction(
														"action.chord.general.duplicate")
										+ "' to copy notes on current position",
								leftSpacing,
								this.selectedMeasure.getTs().getPosition(
										TGTrackSpacing.POSITION_TOP), false);
					}
