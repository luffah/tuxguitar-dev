...
public class KeyBindingActionManager {
	...
	protected class KeyBindingListener implements KeyListener {
		...
		public void keyPressed(KeyEvent event) {
			...
			//LUF201512 workaround to detect digits for French keyboard
			int c = (int) ((KeyEvent) event).character;
			if ( (c >=  KeyBindingConstants.NUMBER_0)
				&& (c <= KeyBindingConstants.NUMBER_9) ) {
				event.keyCode=c;
				event.stateMask=0;
			} //LUF201512 workaround end
			kb.setKey(event.keyCode);
			kb.setMask(event.stateMask);
			//LUF201512 system output to ease keybinding settings	
	     	System.out.println("KeyBindingListener get : "+kb.toString() + " ( key=\"" + kb.getKey() + "\" mask=\"" + kb.getMask() +"\" )");
	     	//LUF201512 end system output to ease keybinding	
	     	
			Action action = getActionForKeyBinding(kb);
			if (action != null){
				//LUF201512 system output to ease keybinding settings	
		     	System.out.println("KeyBindingActionManager process : "+ action.getName());
		     	//LUF201512 end system output to ease keybinding	
